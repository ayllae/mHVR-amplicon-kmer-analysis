...
def extract_sample_id(read_id):#Extract the sample ID from a read ID formatted as: "Sample1_month_year_Read1234"
#This returns the part before the first underscore: "Sample1"
    return read_id.split("_", 1)[0]

def process_record(record): #Process one read and return: k-mer count vector, complete read ID and extracted sample ID.
    read_id=record.id
    return(
        kmer_freq_fast(str(record.seq)),
        read_id,
        extract_sample_id(read_id)
    )
